    ...
    begin
      ...
      if(sl.present? && sl.expire_date >= Date.today)
        ...
        if !bot_request?
          ...
          trc = Track.where(referer: request.referer,ip_address: remote_ip, short_link_id: sl.id).where(clicked_at: (Time.now.midnight - 1.day)..Time.now.midnight).first
          if(trc.present?)
            trc.update_attributes(clicks: trc.clicks + 1)
          else
            track = Track.create(referer: request.referer,ip_address: remote_ip,clicks: 1,clicked_at: Time.now,short_link_id: sl.id,user_agent: request.env["HTTP_USER_AGENT"],country_code: country_code )
          end
